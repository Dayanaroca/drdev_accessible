...
?>
<article <?php post_class( 'accesible-seccion-card bg-white overflow-hidden border-card hover:shadow-2xl rounded-custom' ); ?>>
...
<?php endif; ?>
    <div class="accesible-seccion flex flex-col gap-3 justify-between w-full px-3 lg:px-5 pt-2 pb-3 lg:pt-3 lg:pb-5 bg-color1 shadow-black">
        ...
        <?php 
                $link = get_permalink();
                $text = esc_html__( 'Ver detalles', 'drdevcustomlanguage' );
                echo drdev_link('btn-primary ', $text, $link, '', '', '', '');
            ?>
    </div>
</article>