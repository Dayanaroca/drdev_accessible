...
$not_include_icon = drdev_inline_svg('/assets/images/icons/not_include.svg');
?>
<?php if ($travel['not_includes_travel']) : ?>
<section class="flex flex-col gap-4 w-full max-w-screen-etg mx-auto px-2.5 lg:px-0">   
    ...
    <ul class="flex flex-col gap-3" role="list">
        <?php foreach ( $travel['not_includes_travel'] as $item ) : ?>
            <?php if ( empty( $item['text_not_includes_travel_'] ) ) continue; ?>
            <li class="flex flex-row gap-3 items-start">
                <span class="mt-1 shrink-0" aria-hidden="true">
                    <?php echo $not_include_icon; ?>
                </span>
                <p class="text-lg text-baseblack font-medium">
                    <?php echo esc_html( $item['text_not_includes_travel_'] ); ?>
                </p>
            </li>
        <?php endforeach; ?>
    </ul>
